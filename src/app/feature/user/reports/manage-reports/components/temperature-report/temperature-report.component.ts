...
@Component({
  ...
  styleUrl: './temperature-report.component.scss'
})
export class TemperatureReportComponent {
  ...
  confirm(event: any, type: any) {
    if (type == 'FromDate') {
      this.temperatureForm.controls['fromDate'].patchValue(event?.dateTime);
    } else if (type == 'ToDate') {
      this.temperatureForm.controls['toDate'].patchValue(event?.dateTime);
    }
  }

...
@Component({
  ...
  templateUrl: './overspeed-report.component.html',
  styleUrl: './overspeed-report.component.scss'
})
export class OverspeedReportComponent {
  ...
  confirm(event: any, type: any) {
    if (type == 'FromDate') {
      this.speedForm.controls['fromDate'].patchValue(event?.dateTime);
    } else if (type == 'ToDate') {
      this.speedForm.controls['toDate'].patchValue(event?.dateTime);
    }
  }

...
@Component({
  ...
  styleUrl: './stoppage-report.component.scss'
})
export class StoppageReportComponent {
  ...
  confirm(event: any, type: any) {
    if (type == 'FromDate') {
      this.stopForm.controls['fromDate'].patchValue(event?.dateTime);
    } else if (type == 'ToDate') {
      this.stopForm.controls['toDate'].patchValue(event?.dateTime);
    }
  }

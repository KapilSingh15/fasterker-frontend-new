...
@Component({
  ...
  styleUrl: './ac-report.component.scss'
})
export class AcReportComponent {
  isLoading: boolean = true
  acReportForm!: FormGroup
  ...
  /*For Form Control*/
  setInitialForm() {
    this.acReportForm = this.fb.group({
  ...
  confirm(event: any, type: any) {
    if (type == 'FromDate') {
      this.acReportForm.controls['fromDate'].patchValue(event?.dateTime);
    } else if (type == 'ToDate') {
      this.acReportForm.controls['toDate'].patchValue(event?.dateTime);
    }
  }
